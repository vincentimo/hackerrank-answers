...
func countApplesAndOranges(s int32, t int32, a int32, b int32, apples []int32, oranges []int32) {
  ...
  var i32CurrentPosition int32
  var iAppleCount int = 0
  var iOrangeCount int = 0
  
  // Do the Apple loop
  for i := 0; i < len(apples); i++ {
    i32CurrentPosition = a + apples[i]
    if (i32CurrentPosition >= s) && (i32CurrentPosition <= t) {
      iAppleCount++
    }
  }
  ...
  for i := 0; i < len(oranges); i++ {
    i32CurrentPosition = b + oranges[i]
    if (i32CurrentPosition >= s) && (i32CurrentPosition <= t) {
      iOrangeCount++
    }
  }
